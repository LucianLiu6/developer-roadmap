...
import { c as cn, f as httpPost } from './BaseLayout_CyQdLUYI.mjs';
...
function CreateRoadmapModal(props) {
  ...
  async function handleSubmit(e, redirect = true) {
    ...
    const { response, error } = await httpPost(
      `${{"BASE_URL": "/", "MODE": "production", "DEV": false, "PROD": true, "SSR": true, "SITE": "https://roadmap.sh/", "ASSETS_PREFIX": undefined}.PUBLIC_API_URL}/v1-create-roadmap`,
      {
        title,
        description,
        ...teamId && {
    ...
    if (redirect) {
      window.location.href = `${{"BASE_URL": "/", "MODE": "production", "DEV": false, "PROD": true, "SSR": true, "SITE": "https://roadmap.sh/", "ASSETS_PREFIX": undefined}.PUBLIC_EDITOR_APP_URL}/${response?._id}`;
      return;
    }
    if (onCreated) {
      onCreated(response);
